...
namespace EDMApiWebRole.Controllers
{
    ...
    public class APCAInvoiceController : ApiController
    {
       ...
       /* public HttpResponseMessage GetAPInvoice(string recordnumber)
        ...
        [System.Web.Http.ActionName("Metadata")]
        public HttpResponseMessage DeleteAPCAInvoice(string recordnumber)
        {
            try
            {
                if (!String.IsNullOrEmpty(recordnumber))
                {
                    var apcainvoices = from m in db.APCAInvoices where (m.Record_Number == recordnumber) select m;
                    if (apcainvoices.Count() > 0)
                    {
                        db.APCAInvoices.Remove(db.APCAInvoices.Single(x => x.Record_Number == recordnumber));
                        db.SaveChanges();
       ...
       public HttpResponseMessage PutAPInvoice(string recordnumber, string invoicedate, string invoicenumber, string vendornumber, string vendorname, string ponumber, string invoicetypecd)
        ...
        public async Task<HttpResponseMessage> PostAPCAInvoiceImage(string recordnumber, string fileformat)
        {
            try
            {
                if (!String.IsNullOrEmpty(recordnumber))
        ...
        [System.Web.Http.ActionName("Image")]
        public HttpResponseMessage DeleteAPCAInvoiceImage(string recordnumber)
        {
            try
            {
        ...
        private CloudBlockBlob GetBlockBlob(string recordnumber)
        {
            ...
            CloudStorageAccount storageAccount = CloudStorageAccount.Parse(
                ConfigurationManager.ConnectionStrings["APCAStorageConnectionString"].ConnectionString);
            // Create the blob client.
            CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();

            String containerName = ConfigurationManager.ConnectionStrings["APCAContainerConnectionString"].ConnectionString;
